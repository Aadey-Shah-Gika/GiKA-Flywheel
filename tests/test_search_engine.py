...
def search_engine_worker(task_queue, submit_queue):
    with Manager() as manager:
        scraper = GoogleSearchWebScraper(manager=manager, task_queue=task_queue, submit_task_queue=submit_queue)
        scraper.start()
...
def collect_results_worker(task_queue, max_tasks):
    ...
    while len(results) < max_tasks:
        ...
        with open("./tests/data/load_balancer/results.json", "w", encoding="utf-8") as file:
            json.dump(results, file, indent=4)
def enqueue_task(queries, task_queue, thread_id):
    for query in queries:
        print(f"[ENQUEUE] -- [THREAD_ID:{thread_id}] -- [SENDING TASK] :: CURRENT:", query)
...
def test_search_engine():
    ...
    search_engine_process = Process(target=search_engine_worker, args=(task_queue, submit_queue,))
    ...
    for num in range(num_enqueue_threads):
        ...
        thread = Process(target=enqueue_task, args=(cur_queries, task_queue, num,))
        thread.start()
        enqueue_threads.append(thread)
    
    result_collector_process = Process(target=collect_results_worker, args=(submit_queue, len(queries)))
